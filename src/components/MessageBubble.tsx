...
function VoiceMessage({ url, duration, isKitten }: { url: string | null; duration: number; isKitten: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 py-1 ${isKitten ? 'text-pink-900' : ''}`}>
      <button
        onClick={toggle}
        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 transition-all active:scale-90 ${
          isKitten ? 'bg-pink-300 text-pink-900' : 'bg-theme/20 text-theme'
        }`}
      >
          ...
          return (
            <div
              ...
              className={`w-0.5 rounded-full transition-colors ${
                filled
                  ? isKitten ? 'bg-pink-500' : 'bg-theme'
                  : isKitten ? 'bg-pink-200' : 'bg-theme/30'
              }`}
              style={{
                height: `${30 + Math.sin(i * 1.5) * 40 + 30}%`,
              }}
            />
          );
        })}
      </div>
      <span className={`text-xs shrink-0 ${isKitten ? 'text-pink-700' : 'text-ink-muted'}`}>
        {formatDuration(playing ? currentTime : actualDuration)}
      </span>
    </div>
  );
}
